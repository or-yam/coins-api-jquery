class ApiManager {
  ...
  toggleSave(id: Coin['id'], value: boolean) {
    let favoritesCount: number = this.data.coins.filter((c) => c.isSaved)
      .length;
    let coin: Coin = this.data.coins.find((coin) => coin.id === id)!;
  ...
  getDataToCanvas = async () => {
    //get data from API
    let favorites = this.data.coins
      .filter((c) => c.isSaved)
      .map((f) => f.symbol);
    let graphAjaxData = await $.ajax({
      method: 'GET',
      url: `https://min-api.cryptocompare.com/data/pricemulti?fsyms=${favorites}&tsyms=USD`,
      dataType: 'json',
      error: (text, error) => ({ error: text }),
      success: (graphCoins: GraphCoins[]) => graphCoins,
    });
